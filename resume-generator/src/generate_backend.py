...
def generate(output_path: str = "Arkadiy_Pechnikov_Resume_Backend.pdf"):
    ...
    story.append(Paragraph(data["titles"]["backend"], styles['JobTitleHeader']))
    ...
    story.append(Paragraph(data["summaries"]["backend"], styles['Summary']))

    # Skills
    story.append(Paragraph("SKILLS", styles['SectionHeader']))
    skill_keys = ["languages", "ml_data", "backend", "blockchain", "infrastructure", "monitoring", "databases"]
    ...
    doc.build(story)
    print(f"Backend resume created: {output_path}")
